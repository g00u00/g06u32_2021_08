...
def py_form_processing(db,cur):
    ...
    if (form.getvalue("qs_price") == "0"):# очищаем файл и даляем таблицу
        app_to_file = open(file_name, mode='w', encoding="utf-8", errors=None, newline=None, closefd=True, opener=None)
        cur.execute("""DROP TABLE IF EXISTS `qs_file`""")
        
        
        
    else:# дозаписывем в файл
    ...
    with open(file_name, mode='a', encoding="utf-8", errors=None, newline=None, closefd=True, opener=None) as app_to_file:
        ...
        for form_key in form_keys:
            ...
            app_to_file.write(',"%s": "%s"' % (str(form_key), str(value)))
        app_to_file.write("}}},\n")
    print('<a href="',file_name, '" target="blank" >','Содержание файла с записями</a>\n<br>', sep='', end="\n")
    
    
    print("\n<p><br>Пишем в базу и считываем из базы",  end="\n<br>")
    cur.execute("""CREATE TABLE IF NOT EXISTS `qs_file` (
    ...
    cur.execute("""INSERT INTO `qs_file` (`id`, `qs_file_name`, `qs_file_price`, `qs_file_amount`, `qs_file_type`, `qs_file_date`, `qs_file_time`) 
    ...
    cur.execute("""SELECT * FROM `qs_file` ORDER BY `id` DESC LIMIT 0 , 1;""")
    ...
    print("\n\n<p>Поля в таблице:")
    cur.execute("""SHOW COLUMNS FROM `qs_file`;""")
    ...
    cur.execute("""SELECT * FROM `qs_file`;""")
    result_all  =  cur.fetchall()
    print("\n<br>result_all:\n<br>", result_all)
    ...
    cur.execute("""SELECT `qs_file_type`, COUNT(`qs_file_type`), SUM( qs_file_price * qs_file_amount ) AS `gr_sum` FROM `qs_file` GROUP BY `qs_file_type` ORDER BY `gr_sum` DESC;""")
    result_all  =  cur.fetchall()
    print("\n<br>result_all:\n<br>", result_all)
    ...
    cur.execute("""SELECT sum( qs_file_price * qs_file_amount )
        FROM `qs_file`  WHERE `qs_file_type` LIKE 'Доходы';""")
    result_one  =  cur.fetchone()
    revenue=result_one[0]
    cur.execute("""SELECT sum( qs_file_price * qs_file_amount )
        FROM `qs_file`  WHERE `qs_file_type` LIKE 'Расходы';""")
    result_one  =  cur.fetchone()
